...
function SideNavbar() {
    ...
    return (
        <main>
            <aside className='w-52 2xl:w-64 h-screen bg-white p-2 pt-4 border-r border-gray-300 shadow-2xl '>
                    ...
                    <div className='flex flex-col'>
                        ...
                        <div className='flex flex-col gap-4'>
                            {navItems.map((item)=>(
                                ...
                                key={item.id} className={`flex items-center gap-2 p-2 rounded-md cursor-pointer ${isActive === item.id ? 'bg-blue-500 text-white' : 'bg-transparent text-gray-700 hover:bg-gray-200'}`}>
                                    <span >{item.icon}</span>
